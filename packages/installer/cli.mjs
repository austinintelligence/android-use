...
async function readBytes(source, limit) {
...
async function readManifest(options) {
  ...
  const raw = await readBytes(source, MAX_MANIFEST_BYTES);
  let manifest;
  try {
...
async function rollback(options) {
  ...
  if (!options.dryRun) {
    ...
    await writeJsonAtomic(p.current, { product: PRODUCT, version: previous, host: true, rollback: true });
  }
