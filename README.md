...
From the release installer on Windows x64:
...
```powershell
cargo fmt --manifest-path scripts/rust/Cargo.toml -- --check
cargo clippy --manifest-path scripts/rust/Cargo.toml --all-targets --all-features -- -D warnings
cargo test --manifest-path scripts/rust/Cargo.toml --all-targets
cargo build --manifest-path scripts/rust/Cargo.toml --release
...
See [`docs/installation.md`](docs/installation.md), [`docs/architecture.md`](docs/architecture.md), and [`references/command-map.md`](references/command-map.md).
