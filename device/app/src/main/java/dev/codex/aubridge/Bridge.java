...
final class Bridge implements AutoCloseable {
    ...
    private void listen(boolean boot){try{LocalServerSocket server=new LocalServerSocket(boot?"aubridge-bootstrap":"aubridge");if(boot)bootstrap=server;else command=server;while(open){LocalSocket socket=server.accept();try{workers.execute(()->handle(socket,boot));}catch(RuntimeException e){try{socket.close();}catch(IOException ignored){}}}}catch(IOException ignored){}}
