...
The contract helper path also accepts `--contract` and returns bounded `choices` with
an opaque stable `ref`, role, label, visibility, bounds, and redaction state.
The host falls back to legacy compact rows when an older helper is installed,
so upgrading the host does not invalidate the existing helper wire format.
...
The contract executor generalizes that fast path: adjacent `find`, `tap`, `long`,
