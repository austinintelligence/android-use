...
$schemaPath = Join-Path $root 'references\agent-contract.json'
$generatedPath = Join-Path $root 'skills\android-use\references\agent-contract.json'
...
if ((Compare-Object (Get-Content -LiteralPath $schemaPath -Raw) (Get-Content -LiteralPath $generatedPath -Raw))) {
  throw 'public generated agent contract is not synchronized with references/agent-contract.json'
}
