...
$rows = @(
  ...
  [pscustomobject]@{ id='E1'; variable='Execute-first daemon and config cache'; status='PARTIAL'; correctness='PASS'; first_pass='not isolated'; usb_p95_ms=$usbNoopP95; wifi_p95_ms=$wifiNoopP95; exact_tokens='UNAVAILABLE'; evidence=@($usbNativePath,$wifiNativePath,'scripts/rust/src/daemon.rs','scripts/rust/src/config.rs'); note='Persistent path is measured, but no clean E0/E1-only checkout comparison was retained.' },
  [pscustomobject]@{ id='E2'; variable='Cached device inventory and event tracking'; status='PARTIAL'; correctness='PASS'; usb_p95_ms=$null; wifi_p95_ms=$null; exact_tokens='UNAVAILABLE'; evidence=@('scripts/rust/src/device.rs','references/device-selection.md','artifacts/final/evidence-matrix.md'); note='Exact-serial selection/failover and event-aware helper state are tested; isolated latency attribution is unavailable.' },
  [pscustomobject]@{ id='E3'; variable='Persistent helper forward and socket'; status='PARTIAL'; correctness='PASS'; usb_p95_ms=$null; wifi_p95_ms=$null; exact_tokens='UNAVAILABLE'; evidence=@('scripts/rust/src/helper.rs','references/helper-install.md','artifacts/final/evidence-matrix.md'); note='Live restart/recovery passed on both transports; no isolated model-cost run was captured.' },
  [pscustomobject]@{ id='E4'; variable='Persistent CDP forward and WebSocket'; status='PARTIAL'; correctness='PASS'; usb_p95_ms=$null; wifi_p95_ms=$null; exact_tokens='UNAVAILABLE'; evidence=@('scripts/rust/src/web.rs','references/web-cdp.md','artifacts/final/evidence-matrix.md'); note='CDP reuse and cleanup passed live; no isolated A/B timing ledger exists.' },
  ...
  [pscustomobject]@{ id='E16'; variable='Lean packaging and deployment'; status='MEASURED'; correctness='PASS'; usb_p95_ms=$null; wifi_p95_ms=$null; exact_tokens='UNAVAILABLE'; evidence=@('scripts/build-helper.ps1','scripts/generate-skill.ps1','artifacts/rollback','artifacts/final/rollback-validation-2026-08-01.md','artifacts/final/evidence-matrix.md'); note='Pinned toolchain, generated skill, hashes, signed APK, rollback copies, and a live installed-root candidate swap/restore are verified.' }
)

$report = [pscustomobject]@{
  generated_at = (Get-Date).ToUniversalTime().ToString('o')
  candidate_binary_sha256 = (Get-FileHash -LiteralPath (Join-Path $Root 'scripts\rust\target\release\au.exe') -Algorithm SHA256).Hash.ToLowerInvariant()
