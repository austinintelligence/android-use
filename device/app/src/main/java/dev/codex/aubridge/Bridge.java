...
final class Bridge implements AutoCloseable {
    ...
    Bridge(Context context){this.context=context;new SecureRandom().nextBytes(token);new SecureRandom().nextBytes(nonce);capture=new Capture(context);vm=new Vm(context,capture);}
    ...
    private static JSONArray apps(Context context){JSONArray out=new JSONArray();Intent intent=new Intent(Intent.ACTION_MAIN);intent.addCategory(Intent.CATEGORY_LAUNCHER);java.util.HashSet<String> seen=new java.util.HashSet<>();for(ResolveInfo info:context.getPackageManager().queryIntentActivities(intent,0)){String pkg=info.activityInfo==null?"":info.activityInfo.packageName;if(pkg.isEmpty()||seen.contains(pkg))continue;String label=info.loadLabel(context.getPackageManager())==null?pkg:info.loadLabel(context.getPackageManager()).toString();if(label.length()>128)label=label.substring(0,128);out.put(new JSONArray().put(pkg).put(label));seen.add(pkg);if(out.length()>=128)break;}return out;}
    ...
    @Override public void close(){open=false;try{if(bootstrap!=null)bootstrap.close();}catch(IOException ignored){}try{if(command!=null)command.close();}catch(IOException ignored){}workers.shutdownNow();}
