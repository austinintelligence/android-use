...
$rows = @(
  ...
  [pscustomobject]@{ id='E9'; variable='Token-tape vocabulary'; status='MEASURED'; correctness='PASS'; usb_p95_ms=$null; wifi_p95_ms=$null; exact_tokens='PROXY_ONLY'; evidence=@('crates/android-use/src/tape.rs','references/tape-protocol.md','artifacts/final/codec-evaluation.json'); note='Five complete corpus cases compare compatibility JSON, compact JSON, and tape; exact Luna tokenizer is unavailable.' },
  [pscustomobject]@{ id='E10'; variable='Session dictionary'; status='MEASURED'; correctness='PASS'; usb_p95_ms=$null; wifi_p95_ms=$null; exact_tokens='PROXY_ONLY'; evidence=@('crates/android-use/src/tape.rs','references/tape-protocol.md','artifacts/final/evidence-matrix.md'); note='Dictionary epoch/checksum/reset and daemon-session reuse passed; no exact model token ledger.' },
